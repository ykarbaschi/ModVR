...
public class GizmoHandle : MonoBehaviour, IBeginDragHandler, IDragHandler, IPointerClickHandler
{
    ...
    public void OnDrag(PointerEventData data)
    {
        ...
        if (activeHandle)
        {
            ...
            horz = data.delta.x;// * Mathf.Sign(Vector3.Dot(Camera.main.transform.right, Vector3.right));
            vert = data.delta.y;// * Mathf.Sign(Vector3.Dot(Camera.main.transform.up, Vector3.up));
                ...
                case GizmoTypes.Position:
                    ...
                    switch (Axis)
                    {
                        ...
                        case GizmoAxis.Y:
                            ...
                            {
                                //obj.Translate(Vector3.up * delta, Space.World);
                                obj.Translate(Vector3.up * vert, Space.World);
                            }
                            break;
                        case GizmoAxis.Z:
                            foreach (var obj in Gizmo.SelectedObjects)
                                obj.Translate(Vector3.forward * (horz+vert), Space.World);
                            break;
                        case GizmoAxis.Center:
                            // Based on the camera position we need to either move X horizontal or vertical / vice versa with Z
                            foreach (var obj in Gizmo.SelectedObjects)
                            {
                                ...
                                obj.Translate(Camera.main.transform.up * vert, Space.World);
                            }
                            break;
                    }
                    break;
